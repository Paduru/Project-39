...
function draw() {
  ...
  if (gameState === PLAY) {
    ...
    //move the ground
    ground.velocityX = -5 - Math.floor(score / 100);
    //scoring
    score = score + Math.round(getFrameRate() / 60);

    if (ground.x < camera.position.x) {
      ground.x = camera.position.x + 900;
    }
...
function spawnClouds() {
  ...
  if (frameCount % 60 === 0) {
    cloud = createSprite(camera.position.x + 300, 0, 40, 10);
    cloud.y = Math.round(
      random(camera.position.y - 75, camera.position.y - 25)
    );
